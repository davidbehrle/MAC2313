...
\newcommand\comp[2]{\text{comp}_{\vec{#1}}{\vec{#2}}}
...
\begin{center}
    \begin{tikzpicture}
        ...
        \node[fancytitle,right=10pt] at (box.north west) {Definition of Scalar Multiplication};
    \end{tikzpicture}
\end{center}
...
\begin{center}
    \begin{tikzpicture}
        \node[mybox] (box){
            \begin{minipage}{0.95\textwidth}
                \noindent Let $\vec{v}$ be a vector with initial point $(x_i,y_i)$ and terminal point $(x_t,y_t)$, $\vec{v}$ can be expressed in component form as 
                \begin{align*}
...
\begin{center}
    \begin{tikzpicture}
        ...
        \node[fancytitle,right=10pt] at (box.north west) {Properties of Vector Operations};
    \end{tikzpicture}
\end{center}
\begin{center}
    \begin{tikzpicture}
        ...
        \node[fancytitle,right=10pt] at (box.north west) {Proof of Commutative \& Distributive Properties};
    \end{tikzpicture}
\end{center}
\newpage\noindent
...
\begin{center}
    \begin{tikzpicture}
        ...
        \node[fancytitle,right=10pt] at (box.north west) {The Distance Between Two Points in Space};
    \end{tikzpicture}
\end{center}
\newpage
...
\begin{center}
    \begin{tikzpicture}
        \node[mybox] (box){
            \begin{minipage}{0.95\textwidth}
                A sphere with center $(a,b,c)$ and radius $r$ can be represented by the equation
        ...
        \node[fancytitle,right=10pt] at (box.north west) {Equation of a Sphere};
    \end{tikzpicture}
\end{center}
\newpage
...
\begin{center}
    \begin{tikzpicture}[x={(-0.8562cm,-0.3116cm)},y={(0.5166cm,-0.5166cm)},z={(0cm,0.7975cm)},scale=1.5]
        \draw[densely dashed,fg4] (-0.666,1.333,0.166)--(-0.666,1.333,0)--(-0.666,0,0)--(-0.666,0,0.166)--(0,0,0.166)--(0,1.333,0.166)--(0,1.333,0)--(-0.666,1.333,0);
        \draw[->,very thick,-latex,gbred] (0,0,0)--(-0.666,1.333,0.166);
        \draw[densely dashed,fg4] (0,1.333,0.167)--(-0.666,1.333,0.166)--(-0.666,0,0.166);
        \node[right] at (-0.7,1.4,0.2){$\vec{v} = \la -2, 4, 1 \ra$};
        \draw[<->,latex-latex] (-2,0,0)--(2,0,0)node[left]{$x$};
        \draw[<->,latex-latex] (0,-2,0)--(0,2,0)node[below,xshift=1mm]{$y$};
        \draw[<->,latex-latex] (0,0,-2)--(0,0,2)node[above]{$z$};
    \end{tikzpicture}
\end{center}
...
The first type of vector multiplication is called the dot product because of the notation used and is defined as follows:
\begin{center}
    \begin{tikzpicture}
        \node[mybox] (box){
            \begin{minipage}{0.95\textwidth}
                \setcounter{equation}{2}
                Let $\vec{u} = \la u_1, u_2, u_3 \ra$ and $\vec{v} = \la v_1, v_2, v_3 \ra$ be vectors, $\vec{u},\vec{v} \in \mathbb{R}$. The dot product $\vec{u} \cdot \vec{v}$ is defined as
                \begin{align}
    ...
    \end{tikzpicture}
\end{center}
\noindent When two vectors are added or subtracted, the result is a vector; the dot product of two vectors is a scalar. Like addition and subtraction, the dot product has several algebraic properties.
\begin{center}
    \begin{tikzpicture}
        \node[mybox] (box){
            \begin{minipage}{0.95\textwidth}
                ...
                \begin{align*}
                    \text{i}&. &\vec{u} \cdot \vec{v} &= \vec{v} \cdot \vec{u} &\text{Commutative Property}\\
                    \text{ii}&. &\vec{u} \cdot (\vec{v} + \vec{w}) &= \vec{u} \cdot \vec{v} + \vec{u} \cdot \vec{w} &\text{Distributive Property}\\
                    \text{iii}&. &c(\vec{u} \cdot \vec{v}) &= (c\vec{u}) \cdot \vec{v} = \vec{u} \cdot (c\vec{v}) &\text{Associative Property}\\
                    \text{iv}&. &\vec{v} \cdot \vec{v} &= ||\vec{v}||^2 &\text{Property of Magnitude}
                \end{align*}
            \end{minipage}
        };
        \node[fancytitle,right=10pt] at (box.north west) {Properties of the Dot Product};
    \end{tikzpicture}
\end{center}
...
\begin{center}
    \begin{tikzpicture}[scale=2]
        \draw (0,0) coordinate (a);
        \draw ({sqrt(3)},1) coordinate (b);
        \draw (0.7,{(1.25*sqrt(3))/2}) coordinate (c);
        \draw[->,thick,-latex] (a)--node[below]{$\vec{v}$}(b);
        \draw[->,thick,-latex] (a)--node[above,xshift=-2mm]{$\vec{u}$}(c);
        \draw pic["$\theta$",draw=fg1,-,thick,angle eccentricity=1.5,angle radius=5mm]{angle=b--a--c};
    \end{tikzpicture}
\end{center}
...
\begin{center}
    \begin{tikzpicture}[scale=2]
        ...
        \draw[->,thick,-latex] (a)--node[below]{$\vec{v}$}(b);
        \draw[->,thick,-latex] (a)--node[above,xshift=-2mm]{$\vec{u}$}(c);
        \draw[->,thick,-latex] (c)--node[above]{$\vec{v} - \vec{u}$}(b);
        \draw pic["$\theta$",draw=fg1,-,thick,angle eccentricity=1.5,angle radius=5mm]{angle=b--a--c};
    \end{tikzpicture}
\end{center}
...
\begin{center}
    \begin{tikzpicture}
        \node[mybox] (box){
            \begin{minipage}{0.95\textwidth}
                Two nonzero vectors $\vec{u}$ and $\vec{v}$ are orthogonal if and only if $\vec{u} \cdot \vec{v} = 0$\\
                Since $\displaystyle\cos(\theta) = \frac{\vec{u} \cdot \vec{v}}{|\vec{u}||\vec{v}|}$, $\vec{u} \cdot \vec{v} = 0$ if and only if $\theta = \displaystyle\frac{\pi}{2}$
            \end{minipage}
        };
        \node[fancytitle,right=10pt] at (box.north west) {Theorem: Orthogonal Vectors};
    \end{tikzpicture}
\end{center}
In practical computations, the angle a vector makes with each of the coordinate axes - its direction angles - are very important. Direction angles are often calculated using the dot product and the cosines of the angles, called direction cosines.
\begin{center}
    \begin{tikzpicture}[scale=1.5]
        \draw[->,-latex,thick] (0,0)--(1.75,0);
        \draw[->,-latex,thick] (0,0)--(0,1.75);
        \draw[->,-latex,thick] (0,0)--({-1.75*cos(45)},{-1.75*sin(45)});
        \draw (0,0) coordinate (o);
        \draw (0,1) coordinate (a);
        \draw (1,0) coordinate (b);
        \draw ({-cos(45)},{-sin(45)}) coordinate (c);
        \draw ({1.5*cos(60)},{1.5*sin(60)}) coordinate (d);
        \draw[->,-latex,very thick,gbblue2] (0,0)--(a);
        \draw pic["$\alpha$",draw=fg1,-,thick,angle eccentricity=1.5,angle radius=2.5mm]{angle=d--o--c};
        \draw pic["$\beta$",draw=fg1,-,thick,angle eccentricity=1.5,angle radius=4mm]{angle=b--o--d};
        \draw pic["$\gamma$",draw=fg1,-,thick,angle eccentricity=1.5,angle radius=5mm]{angle=d--o--a};
        \draw[->,-latex,very thick,gbred] (0,0)--(d);
        \draw[->,-latex,very thick,gbblue2] (0,0)--(b);
        \draw[->,-latex,very thick,gbblue2] (0,0)--(c);
    \end{tikzpicture}
\end{center}
For example, the direction cosines of the vector $\vec{v} = \la 2, 3, 3 \ra$ are $\cos(\alpha) = \frac{2}{\sqrt{22}}$, $\cos(\beta) = \frac{3}{\sqrt{22}}$, and $\cos(\gamma) = \frac{3}{\sqrt{22}}$. For any vector $\vec{u} = \la a, b, c \ra$ its direction cosines are given by $\cos(\alpha) = \frac{a}{|\vec{u}|},\ \cos(\beta) = \frac{b}{|\vec{u}|}$, and $\cos(\gamma) = \frac{c}{|\vec{u}|}$
\newpage
\noindent\bf{Projections}
\begin{center}
    \begin{tikzpicture}
        \node[mybox] (box){
            \begin{minipage}{0.95\textwidth}
                The vector projection of $\vec{v}$ onto $\vec{u}$ is denoted $\proj{u}{v}$. It has the same initial point as $\vec{u}$ and $\vec{v}$, and the same direction as $\vec{u}$, and represents the component of $\vec{v}$ acting in the direction of $\vec{u}$. If $\theta$ is the angle between $\vec{u}$ and $\vec{v}$, then the length of $\proj{u}{v}$ is $|\proj{u}{v}| = |\vec{v}|\cos(\theta)$.
                \vspace{1mm}\\
                Expressing $\cos(\theta)$ in terms of the dot product, this becomes
                \begin{align*}
                    |\proj{u}{v}| &= |\vec{v}|\cos(\theta)\\
                    &= |\vec{v}|\left(\frac{|\vec{u} \cdot \vec{v}|}{|\vec{u}||\vec{v}|}\right)\\
                    &= \frac{|\vec{u} \cdot \vec{v}|}{|\vec{u}|}
                \end{align*}
                This is multiplied by a unit vector in the direction of $\vec{u}$ to give $\proj{u}{v}$
                \setcounter{equation}{5}
                \begin{equation}
                    \proj{u}{v} = \frac{\vec{u} \cdot \vec{v}}{|\vec{u}|}\left(\frac{1}{|\vec{u}|} \vec{u}\right) = \frac{\vec{u} \cdot \vec{v}}{|\vec{u}|^2} \vec{u}
                \end{equation}
                The magnitude of this vector is known as the scalar projection of $\vec{v}$ onto $\vec{u}$ and is denoted by
                \begin{equation}
                    |\proj{u}{v}| = \comp{u}{v} = \frac{\vec{u} \cdot \vec{v}}{|\vec{u}|}
                \end{equation}
            \end{minipage}
        };
        \node[fancytitle,right=10pt] at (box.north west) {Definition};
...
\begin{center}
    \begin{tikzpicture}[scale=1.5]
        \draw[thin,fg4] ({0.15*cos(35)},{0.15*sin(35)})--([turn]90:0.15)--([turn]90:0.15);
        \draw[->,-latex,thick] ({0.7*cos(35)},{0.7*sin(35)})--node[below]{$\vec{u}$}({2*cos(35)},{2*sin(35)});
        \draw[->,-latex,thick,gbblue2] ({0.7*cos(35)},{0.7*sin(35)})--node[below,xshift=4mm,text=fg1]{$\proj{u}{v}$}(0,0);
        \draw[dashed,thick,fg4] (0,0)--({-1.3*cos(55)},{1.3*sin(55)});
        \draw[->,-latex,thick] ({0.7*cos(35)},{0.7*sin(35)})--node[above]{$\vec{v}$} ({-1.3*cos(55)},{1.3*sin(55)});
    \end{tikzpicture}
\end{center}
...
\begin{center}
    \begin{tikzpicture}
        \node[mybox](box){
            \begin{minipage}{0.95\textwidth}
                Let $\vec{u} = \la u_1, u_2, u_3 \ra$ and $\vec{v} = \la v_1, v_2, v_3 \ra$. Then the cross product $\vec{u} \times \vec{v}$ is the vector
                \setcounter{equation}{8}
                \begin{align}
                    \begin{split}
                        \vec{u} \times \vec{v} &= (u_2v_3 - u_3v_2)\ih - (u_1v_3 - u_3v_1)\jh + (u_1v_2 - u_2v_1)\kh \\
                        &= \la u_2v_3 - u_3v_2, -(u_1v_3 - u_3v_1), u_1v_2 - u_2v_1 \ra
                    \end{split}
                \end{align}
            \end{minipage}
        };
